...
export const MissionControlLoader = () => {
  const { t, tArray } = useLanguage();
  const secondary = tArray('missionControl.secondary');
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setSecondaryIndex((prev) => (prev + 1) % secondary.length);
    }, 3500);
  ...
  useEffect(() => {
    ...
    const verticalDuration = 4000;
    const horizontalDuration = 5500;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="mb-6 flex items-center justify-center" style={{ opacity: 0.9 }}>
          <span 
        ...
        <div className="relative mb-8" style={{ width: '200px', height: '120px' }}>
          <div 
            className="absolute inset-0 rounded-lg overflow-hidden"
            style={{
              background: `linear-gradient(90deg, hsl(174 40% 25% / 0.05) 1px, transparent 1px), linear-gradient(hsl(174 40% 25% / 0.05) 1px, transparent 1px)`,
          ...
          <div 
            ...
            style={{
              ...
              boxShadow: '0 0 15px 3px hsl(174 65% 48% / 0.5)',
            }}
          />
          <div 
          ...
          <div className="absolute inset-0 flex items-center justify-around px-4">
            {activeNodes.map((isActive, index) => (
              <div key={index} className="relative" style={{ top: `${(index % 3) * 15 - 15}px` }}>
                <div 
                  className="rounded-full transition-all duration-500"
                  style={{
                    width: '8px',
                    height: '8px',
                    background: isActive ? 'hsl(174 80% 60%)' : 'hsl(220 20% 30%)',
                    boxShadow: isActive ? '0 0 12px 3px hsl(174 70% 55% / 0.8)' : 'none',
                ...
                {isActive && (
                  <div 
                    ...
                    style={{ background: 'hsl(174 70% 55% / 0.3)', animationDuration: '1s' }}
                  />
                )}
              </div>
            ))}
        ...
        <h3 
          ...
          style={{ fontSize: 'clamp(1rem, 0.95rem + 0.3vw, 1.125rem)', color: 'hsl(0 0% 95%)', letterSpacing: '0.02em', textShadow: '0 0 20px hsl(0 0% 100% / 0.2)' }}
        >
          {t('missionControl.primary')}
        </h3>
        <p className="text-center font-sans mb-4" style={{ fontSize: 'clamp(0.75rem, 0.7rem + 0.2vw, 0.85rem)', color: 'hsl(174 50% 70%)', letterSpacing: '0.04em', opacity: 0.9 }}>
          {t('missionControl.subtitle')}
        </p>
        <p 
          key={secondaryIndex}
          className="text-center font-sans animate-fade-in"
          style={{ fontSize: 'clamp(0.7rem, 0.65rem + 0.15vw, 0.8rem)', color: 'hsl(220 15% 55%)', letterSpacing: '0.03em', opacity: 0.7 }}
        >
          {secondary[secondaryIndex]}
        </p>

        <div className="flex gap-2 mt-6" style={{ opacity: 0.6 }}>
          {[0, 1, 2, 3].map((i) => (
            <div
              key={i}
              ...
              style={{
                width: '4px',
                height: '4px',
                background: i === secondaryIndex ? 'hsl(174 70% 55%)' : 'hsl(220 20% 35%)',
                boxShadow: i === secondaryIndex ? '0 0 8px 2px hsl(174 60% 50% / 0.5)' : 'none',
      ...
      <div 
        ...
        style={{ background: 'radial-gradient(ellipse 50% 30% at 50% 30%, hsl(174 60% 45% / 0.08) 0%, transparent 70%)' }}
      />
    </div>
  );
};